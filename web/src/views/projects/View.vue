...
import { dataProjects, dataHeader, dataLegends } from "@/data/projects";
...
let stackedBar = (index) => {
  stackedValue = [];

  for (const grid of dataProjects[index].trackedIssues) {
      ...
      <v-col :cols="9">
        <cs-list :dataHeader="dataHeader">
          <template v-slot:dataList>
            ...
            >
              <cs-list-item :cols="2">{{ project.admin }}</cs-list-item>

              <cs-list-item :cols="2">{{  project.projectName }}</cs-list-item>

              <cs-list-item :cols="6">
                <div id="issuesGraph"
                  :style="{
                    'grid-template-columns': `${stackedBar(index)}`
                  }"
                >
                  <div v-for="legend in project.trackedIssues">
              ...
              <cs-list-item :class="'d-flex justify-center'" :cols="2">
