...
const isLoaded = ref<any>(true);
const projects = ref<any>([]);
const header = ref<any>([projectData.header]);

let stackedValue = ref<any>([]);
let dataFormated = ref<any>([]);
...
const loadProjects = async ({ page, itemsPerPage, sortBy }: any) => {
  isLoaded.value = true;
  await projectStore
    .fetchProjects({ page, itemsPerPage, sortBy })
            ...
            <tr>
              ...
              <td>
                <div
                  id="issuesGraph"
                  :style="{
                    'grid-template-columns': `${stackedBar(item.issues)}`,
                  }"
                >
                  <div
